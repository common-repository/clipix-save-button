...
function clipix_activate() {
		...
		'clipix_horizontal_position' => 'none',
		'clipix_size' => 'medium',
		'clipix_color' => 'gray',
		'language' => 'en'
		);
		delete_option('clipix_setting');
		add_option( 'clipix_setting', $defaults );
}

function Clipix_plugin_setup_menu(){
		//create new top-level menu
        add_menu_page( 'Clipix Plugin Page', 'Save Button', 'manage_options', 'clipix-button-settings', 'Clipix_plugin_setup_init' , plugins_url( 'assets/Clipix_logo16.png', __FILE__ ) );
...
<table class="form-table">   
   ...
   <tr> 
      <th scope="row" style="vertical-align: initial;   width: 200px; text-align: left;">Horizontal position</th>
      <td>
		<label for="clipix_setting[clipix_horizontal_position][none]" class="clipix-radio-label">
      	<input  type="radio" id="clipix_setting[clipix_horizontal_position][none]" name="clipix_setting[clipix_horizontal_position]" value="none" <?php checked('none', $clipix_options['clipix_horizontal_position']); ?> />Default</label>
        <label for="clipix_setting[clipix_horizontal_position][left]" class="clipix-radio-label">
      	<input  type="radio" id="clipix_setting[clipix_horizontal_position][left]" name="clipix_setting[clipix_horizontal_position]" value="left" <?php checked('left', $clipix_options['clipix_horizontal_position']); ?> />Left align</label>
  		<label for="clipix_setting[clipix_horizontal_position][right]" class="clipix-radio-label">
  		<input type="radio" id="clipix_setting[clipix_horizontal_position][right]" name="clipix_setting[clipix_horizontal_position]" value="right" <?php checked('right', $clipix_options['clipix_horizontal_position']); ?> />Right align</label> 
   </tr>
   <tr>
      ...
      <td>
        <label for="clipix_setting[clipix_size][small]" class="clipix-radio-label">
      	<input  type="radio" id="clipix_setting[clipix_size][small]" name="clipix_setting[clipix_size]" value="small" <?php checked('small', $clipix_options['clipix_size']); ?> />Small (20px)</label>
  		<label for="clipix_setting[clipix_size][medium]" class="clipix-radio-label">
  		<input type="radio" id="clipix_setting[clipix_size][medium]" name="clipix_setting[clipix_size]" value="medium" <?php checked('medium', $clipix_options['clipix_size']); ?> />Medium (24px)</label> 
  		<label for="clipix_setting[clipix_size][large]" class="clipix-radio-label">
  		<input type="radio" id="clipix_setting[clipix_size][large]" name="clipix_setting[clipix_size]" value="large" <?php checked('large', $clipix_options['clipix_size']); ?> />Large (32px)<br /></label> 
   </tr>
   <tr> 
      ...
      <td>
      	...
      	<input type="radio" id="clipix_setting[clipix_color][gray]" name="clipix_setting[clipix_color]" value="gray" <?php checked('gray', $clipix_options['clipix_color']); ?> />Gray</label>
      	<label for="clipix_setting[clipix_color][white]" class="clipix-radio-label">
  		<input type="radio" id="clipix_setting[clipix_color][white]" name="clipix_setting[clipix_color]" value="white" <?php checked('white', $clipix_options['clipix_color']); ?> />White</label>  
  		<label for="clipix_setting[clipix_color][orange]" class="clipix-radio-label">
  		<input type="radio" id="clipix_setting[clipix_color][orange]" name="clipix_setting[clipix_color]" value="orange" <?php checked('orange', $clipix_options['clipix_color']); ?> />Orange</label>
      </td>
   </tr>
      <tr> 
...
class Add_Clipix_Save_Button {
	...
	function add_Clipix_Save_Button($content)
	{
		...
		if(is_single() && !is_page( ) && file_exists( $fileName )){
			...
			$horizontalPosition = $clipix_options['clipix_horizontal_position'];
			...
			if ( $horizontalPosition == 'left') {
				$msg = str_replace("{custom_style}", "width: 100%;text-align: left;", $msg);
			}
			
			if ( $horizontalPosition == 'right') {
				$msg = str_replace("{custom_style}", "width: 100%;text-align: right;", $msg);
			}
			
			if ( $horizontalPosition == 'none') {
				$msg = str_replace("{custom_style}", "", $msg);
			}
